...
class PaypalOAuth
{
	...
	public static function getAuthHeaderString($token, $tokenSecret)
	{
		// get api data
		$paypalApiUsername = Config::get('payment.paypal.apiUsername');
		$paypalApiPassword = Config::get('payment.paypal.apiPassword');
		...
		// prepare oauth data
		$oAuthConsumer = new OAuthConsumer($paypalApiUsername, $paypalApiPassword);
		$oAuthSignatureMethod = new OAuthSignatureMethod_HMAC_SHA1();
		$oAuthToken = new OAuthToken($token, $tokenSecret);

		// make request
		$oAuthRequest = OAuthRequest::from_consumer_and_token($oAuthConsumer, $oAuthToken, 'POST', $url);
		$oAuthRequest->sign_request($oAuthSignatureMethod, $oAuthConsumer, $oAuthToken);

		$response = OAuthUtil::parseQueryString($oAuthRequest);
...
// didn't touch the code below this!
