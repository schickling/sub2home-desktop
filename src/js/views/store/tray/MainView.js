define(["jquery", "underscore", "backbone", "services/router", "models/cartModel", "models/stateModel", "views/PageView", "views/store/tray/NoDeliveryView", "views/store/tray/DeliveryTimeView", "views/store/tray/CommentView", "views/store/tray/SubcardView", "views/store/tray/MinimumValueView", "views/store/tray/FreeCookieView", "views/store/tray/ControlView", "views/store/tray/CheckoutSettingsView", "views/store/tray/OrderedItemsView", "text!templates/store/tray/MainTemplate.html"], function($, _, Backbone, router, cartModel, stateModel, PageView, NoDeliveryView, DeliveryTimeView, CommentView, SubcardView, MinimumValueView, FreeCookieView, ControlView, CheckoutSettingsView, OrderedItemsView, MainTemplate) {
  var MainView;
  return MainView = PageView.extend({
    events: {
      "click .settings": "_showCheckoutSettings",
      "hide #checkoutSettings": "_hideCheckoutSettings",
      "click #noDeliveriesToday": "_navigateToHomeHome"
    },
    pageTitle: "Fast fertig - sub2home",
    subViews: {
      controlView: null,
      deliveryTimeView: null,
      minimumValueView: null
    },
    $checkoutSettings: null,
    $trayNote: null,
    initialize: function() {
      if (cartModel.getNumberOfOrderedItems() === 0) {
        router.navigate("store", {
          trigger: true,
          replace: true
        });
        return;
      }
      return this._render();
    },
    _render: function() {
      this.$el.html(MainTemplate);
      this._cacheDom();
      new OrderedItemsView({
        el: this.$("#orderedItems")
      });
      if (this._storeIsDelivering()) {
        this.subViews.controlView = new ControlView({
          el: this.$("#checkoutBasicControls")
        });
        this.subViews.deliveryTimeView = new DeliveryTimeView({
          el: this.$("#deliveryTimeDisplay")
        });
        this.subViews.minimumValueView = new MinimumValueView({
          el: this.$("#minimumOrderValue")
        });
        new CommentView({
          el: this.$("#deliveryAdditionalNote")
        });
        new SubcardView({
          el: this.$("#subcardOption")
        });
        new FreeCookieView({
          el: this.$("#gratisCookieOption")
        });
        new CheckoutSettingsView({
          el: this.$checkoutSettings
        });
      } else {
        new NoDeliveryView({
          el: this.$("#checkoutControls")
        });
        this.$(".goldenThread").hide();
      }
      return this.append();
    },
    _cacheDom: function() {
      this.$checkoutSettings = this.$("#checkoutSettings");
      return this.$trayNote = this.$("#trayNote");
    },
    _showCheckoutSettings: function() {
      var $checkoutSettings, $trayNote, scrollTop;
      $checkoutSettings = this.$checkoutSettings;
      $trayNote = this.$trayNote;
      $checkoutSettings.css({
        height: "auto"
      });
      scrollTop = $trayNote.scrollTop() + $checkoutSettings.height();
      return $trayNote.animate({
        scrollTop: scrollTop
      });
    },
    _hideCheckoutSettings: function() {
      var $checkoutSettings, $trayNote, scrollTop;
      $checkoutSettings = this.$checkoutSettings;
      $trayNote = this.$trayNote;
      scrollTop = $trayNote.scrollTop() - $checkoutSettings.height();
      return $trayNote.animate({
        scrollTop: scrollTop
      }, function() {
        return $checkoutSettings.css({
          height: 0
        });
      });
    },
    _storeIsDelivering: function() {
      return stateModel.get("storeModel").isDeliveringToday();
    },
    _navigateToHomeHome: function() {
      return router.navigate("/", true);
    }
  });
});

/*
//@ sourceMappingURL=MainView.js.map
*/